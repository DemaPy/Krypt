  ...
  return (
    <nav className='w-full flex md:justify-center justify-between p-4'>
        ...
        <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-initial'>
          ...
          <li className="bg-[#2952e3] cursor-pointer rounded-full px-6 py-2 hover:bg-[#2546bd]">
            Login
          </li>
        </ul>
        <div className='flex relative'>
          {toggleMenu
            ? <AiOutlineClose fontSize={28} className="cursor-pointer text-white md:hidden" onClick={() => setToggleMenu(false)}/>
            : <HiMenuAlt4 fontSize={28} className="cursor-pointer text-white md:hidden" onClick={() => setToggleMenu(true)}/>
          }
          { toggleMenu &&
            <ul 
